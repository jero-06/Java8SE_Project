...
public class ManageBook {
    ...
    public static void main(String[] args) {
        ...
        // 3. 가격 변경 및 출력 (7번째 도서 '작별하지않는다' 할인 적용)
        Publication targetBook = publications[6];
        int oldPrice = targetBook.getPrice();
        
        System.out.println("==== 가격 변경 ====");
        System.out.println(targetBook.getTitle() + " 변경 전 가격: " + oldPrice + "원");
        
        modifyPrice(targetBook);
        
        int newPrice = targetBook.getPrice();
        int diffPrice = oldPrice - newPrice;
        
        System.out.println(targetBook.getTitle() + " 변경 후 가격: " + newPrice + "원");
        System.out.println("차액: " + diffPrice + "원");
        System.out.println();
